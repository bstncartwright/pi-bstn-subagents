...
> **BSTN fork note:** this vendored architecture is extended by a
> backend-neutral `ChildSession` boundary. The original Pi implementation and a
> Cursor ACP implementation sit below `Subagent`; queueing, lifecycle state,
> workspaces, observation, notifications, result delivery, and UI remain shared.
> See [`../cursor-acp.md`](../cursor-acp.md).
