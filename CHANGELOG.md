...
- Rebased the standalone package on `@gotgenes/pi-subagents` 18.1.0 at
  upstream commit `67eda5ac9add1cb6bb6240495090b5ecf1a1fb29`.
- Replaced the previous subprocess scheduler, manifest, Herdr viewer, and
  bespoke UI with the upstream native lifecycle, scheduler, service, widget,
  notifications, result retrieval, and transcript navigation.

### Added

- First-class `backend: "cursor"` execution through the official ACP SDK and
  Cursor's `agent acp` server.
- ACP capability negotiation, authentication, model-option discovery,
  permission handling, cancellation, session load/resume negotiation, and
  deterministic process cleanup.
- Backend-neutral activity observation and Cursor text transcripts integrated
  with `/subagents:sessions`.
- Mock ACP integration coverage and an opt-in live Cursor CLI smoke test.
