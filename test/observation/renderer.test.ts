...
describe("buildStatsParts", () => {
  it("includes all parts in order when all fields are present", () => {
    ...
    expect(parts).toEqual(["turn 5/10", "3 tool uses", "1.0k token", "5.0s"]);
  });

  it("omits Pi-style turn counts for Cursor notifications", () => {
    expect(buildStatsParts({
      backend: "cursor",
      turnCount: 5,
      maxTurns: undefined,
      toolUses: 3,
