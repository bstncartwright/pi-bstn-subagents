...
describe("createSubagentSession — post-bind recursion guard", () => {
  ...
  it("does not constructor-filter extension tools before they can register", async () => {
    arrangeFactory();

    await createSubagentSession({ snapshot: STUB_SNAPSHOT, type: "Explore" }, defaultDeps());

    expect(io.createSession.mock.calls[0][0]).not.toHaveProperty("tools");
  });

  it("preserves Codex adapter tools while removing disallowed mutation aliases", async () => {
    const session = arrangeFactory({
      toolsAfterBind: ["exec_command", "write_stdin", "apply_patch", "view_image", "subagent"],
    });
    const registry = createAgentLookup({ builtinToolNames: ["read", "bash"] });
  ...
  it("keeps Codex mutation aliases for agents allowed to edit", async () => {
    const session = arrangeFactory({ toolsAfterBind: ["exec_command", "write_stdin", "apply_patch"] });
    const registry = createAgentLookup({ builtinToolNames: ["read", "bash", "edit", "write"] });
