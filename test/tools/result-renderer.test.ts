...
describe("renderStats", () => {
	...
	it("includes turn count with max turns", () => {
		...
		expect(renderStats(details, theme)).toContain("[dim:turn 5/30]");
	});

	it("includes turn count without max turns", () => {
		const details = makeDetails({ turnCount: 5 });
		expect(renderStats(details, theme)).toContain("[dim:turn 5]");
	});

	it("excludes Pi-style turn counts for Cursor", () => {
		const details = makeDetails({ backend: "cursor", turnCount: 5 });
		expect(renderStats(details, theme)).not.toContain("turn ");
	});
