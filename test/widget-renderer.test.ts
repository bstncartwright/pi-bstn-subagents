...
describe("renderFinishedLine", () => {
	...
	it("renders completed agent with success icon and stats", () => {
		...
		// Turn count with max
		expect(line).toContain("turn 3/10");
	...
	it("renders Cursor explicitly rather than relying on an implicit non-Pi marker", () => {
		const line = renderFinishedLine(makeAgent({ backend: "cursor" }), testRegistry, theme);
		expect(line).toContain("[dim:(cursor)]");
...
describe("renderRunningLines", () => {
	...
	it("returns header and activity lines", () => {
		...
		// Stats: turn count
		expect(header).toContain("turn 2/10");
		// Tool uses
	...
	it("omits Pi-style turn counts for a running Cursor agent", () => {
		const [header] = renderRunningLines(
			makeAgent({ backend: "cursor", status: "running", completedAt: undefined }),
			testRegistry,
			0,
			theme,
		);
...
describe("renderWidgetLines", () => {
	...
	it("renders mixed running + finished + queued agents", () => {
		...
		expect(lines[4]).toContain("1 queued · pi");
		// Total: 5 lines
		expect(lines).toHaveLength(5);
	});

	it("summarizes both backends when mixed agents are queued", () => {
		const lines = renderWidgetLines({
			agents: [
				makeAgent({ id: "pi-q", backend: "pi", status: "queued", completedAt: undefined }),
				makeAgent({ id: "cursor-q", backend: "cursor", status: "queued", completedAt: undefined }),
			],
			registry: testRegistry,
			spinnerFrame: 0,
			terminalWidth: 200,
		...
		expect(lines.join("\n")).toContain("2 queued · 1 pi · 1 cursor");
	});
