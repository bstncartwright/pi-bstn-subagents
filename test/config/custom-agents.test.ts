...
You are a security auditor.`);
  ...
  it("loads Cursor backend frontmatter", () => {
    writeAgent("cursor-reviewer", `---
backend: cursor
...
Review code.`);
    ...
    const agent = loadCustomAgents(tmpDir).get("cursor-reviewer")!;
    expect(agent.backend).toBe("cursor");
    expect(agent.cursorModel).toBe("Composer 2.5");
    expect(agent.permissionMode).toBe("deny");
  });
