  ...
  describe("getToolNamesForType", () => {
    it("preserves an explicit empty tools list", () => {
      const registry = makeRegistry(new Map([
        ["no-tools", makeAgentConfig({ name: "no-tools", builtinToolNames: [] })],
      ]));
      expect(registry.getToolNamesForType("no-tools")).toEqual([]);
    });

    it("uses every built-in only when the list is omitted", () => {
      const registry = makeRegistry(new Map([
        ["all-tools", makeAgentConfig({ name: "all-tools", builtinToolNames: undefined })],
      ]));
      expect(registry.getToolNamesForType("all-tools")).toEqual(BUILTIN_TOOL_NAMES);
    });
  });
