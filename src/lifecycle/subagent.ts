...
export type SteerOutcome =
	| { kind: "delivered" }
	| { kind: "buffered" }
	| { kind: "unsupported"; backend: SubagentBackend }
...
export interface SubagentExecution {
	...
	createSubagentSession: (params: CreateSubagentSessionParams) => Promise<ChildSession>;
	...
	requestPermission?: CreateSubagentSessionParams["requestPermission"];
	maxTurns?: number;
...
export class Subagent {
	...
	async steer(message: string): Promise<SteerOutcome> {
		...
		if (!this.subagentSession.supportsSteer) {
			return { kind: "unsupported", backend: this.subagentSession.backend };
		}
	...
	getTranscript(): ChildTranscript | undefined {
		return this.subagentSession?.getTranscript();
	}

	get messages(): readonly unknown[] {
		const transcript = this.getTranscript();
		return transcript?.kind === "pi" ? transcript.messages : transcript?.entries ?? [];
	}

	get agentMessages(): readonly import("#src/types").SessionMessage[] {
		const transcript = this.getTranscript();
		return transcript?.kind === "pi" ? transcript.messages : [];
	}

	getToolDefinition(name: string) {
		const transcript = this.getTranscript();
		return transcript?.kind === "pi" ? transcript.getToolDefinition(name) : undefined;
	}
	...
	async run(): Promise<void> {
		...
		try {
			this.subagentSession = await this.execution.createSubagentSession({
				...
				backend: this.execution.backend,
				model: this.execution.model,
				cursorModel: this.execution.cursorModel,
	...
	async resume(prompt: string, signal?: AbortSignal): Promise<void> {
		...
		try {
			const resumed = await subagentSession.resumeTurnLoop(prompt, signal);
			const result = typeof resumed === "string"
				? { responseText: resumed, aborted: false, steered: false }
				: resumed;
	...
	disposeSession(): void {
		const result = this.subagentSession?.dispose();
		if (result) void result.catch((error) => debugLog("dispose child session", error));
	}
	...
	releaseSession(): void {
		...
		if (result) void result.catch((error) => debugLog("release child session", error));
