...
import type { CursorPermissionMode, SubagentBackend } from "#src/lifecycle/child-session";
 ...
 */
const CODEX_SHELL_TOOL_NAMES = new Set(["exec_command", "write_stdin", "exec", "wait"]);
const CODEX_MUTATION_TOOL_NAMES = new Set(["apply_patch"]);
const CODEX_ADAPTER_MARKERS = new Set(["exec_command", "apply_patch", "exec"]);

/**
 * Apply the built-in allowlist after extensions initialize.
 *
 * Passing Pi's `tools` option at session construction also filters extension
 * registrations. That breaks providers such as openai-codex whose conversion
 * extension replaces Pi's built-ins with `exec_command` / `apply_patch` during
 * `session_start`. Let extensions register first, then enforce the agent's
 * built-in policy over the resulting active set.
 */
function applyToolPolicy(session: AgentSession, allowedToolNames: readonly string[]): void {
  const allowed = new Set(allowedToolNames);
  const active = session.getActiveToolNames();
  const usesCodexAdapter = active.some((name) => CODEX_ADAPTER_MARKERS.has(name));
  const filtered = active.filter((name) => {
    if (EXCLUDED_TOOL_NAMES.includes(name)) return false;
    if (BUILTIN_TOOL_NAMES.includes(name)) return allowed.has(name);
    if (CODEX_SHELL_TOOL_NAMES.has(name)) return allowed.has("bash");
    if (CODEX_MUTATION_TOOL_NAMES.has(name)) return allowed.has("edit") || allowed.has("write");
    return true;
  });

  // Normal providers retain Pi's requested built-ins. A Codex adapter has
  // intentionally replaced them, so do not re-add duplicate/incompatible
  // schemas after conversion.
  if (!usesCodexAdapter) {
    for (const name of allowedToolNames) {
      if (!filtered.includes(name) && session.getToolDefinition(name)) filtered.push(name);
    }
  }
...
export async function createSubagentSession(
  ...
  try {
    ...
    applyToolPolicy(session, cfg.toolNames);
  } catch (err) {
