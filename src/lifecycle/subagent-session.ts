...
export type { TurnLoopOptions, TurnLoopResult } from "#src/lifecycle/child-session";
 ...
 */
export class SubagentSession implements ChildSession {
  constructor(
  ...
  /** Subscribe to backend-neutral child activity. */
  subscribe(fn: (event: ChildSessionEvent) => void): () => void {
    return this._session.subscribe((event) => {
      if (event.type === "tool_execution_start") {
        fn({ type: "tool_start", toolCallId: event.toolCallId, toolName: event.toolName });
      } else if (event.type === "tool_execution_end") {
        fn({ type: "tool_end", toolCallId: event.toolCallId, toolName: event.toolName, failed: event.isError });
      } else if (event.type === "message_start" && event.message.role === "assistant") {
        fn({ type: "response_start" });
      } else if (event.type === "message_update" && event.assistantMessageEvent.type === "text_delta") {
        fn({ type: "response_delta", text: event.assistantMessageEvent.delta });
        ...
        fn({ type: "compaction", reason: event.reason, tokensBefore: event.result.tokensBefore });
      }
    });
  }
