...
export class SubagentState {
	...
	private _contextPercent: number | null;
	get contextPercent(): number | null { return this._contextPercent; }
	...
	constructor(init: SubagentStateInit = {}) {
		...
		this._contextPercent = init.contextPercent ?? null;
	...
	addActiveTool(toolName: string, toolCallId?: string): void {
		this._activeTools.set(toolCallId ?? toolName + "_" + (++this._toolKeySeq), toolName);
	}

	/** Remove one active tool by name (first match). Called by record-observer on tool_execution_end. */
	removeActiveTool(toolName: string, toolCallId?: string): void {
		if (toolCallId && this._activeTools.delete(toolCallId)) return;
	...
	setContextUsage(used: number, size: number): void {
		this._contextPercent = size > 0 ? Math.max(0, Math.min(100, (used / size) * 100)) : null;
	}
