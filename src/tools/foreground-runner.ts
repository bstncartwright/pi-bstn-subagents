...
export async function runForeground(
  ...
  const streamUpdate = () => {
    ...
    const details: AgentDetails = {
      ...
      turnCount: execution.backend === "pi" ? recordRef?.turnCount ?? 1 : undefined,
      maxTurns: execution.backend === "pi" ? recordRef?.maxTurns ?? execution.effectiveMaxTurns : undefined,
  ...
  try {
    record = await manager.spawnAndWait(
      ...
      execution.prompt,
      {
        ...
        cursorModel: execution.cursorModel,
