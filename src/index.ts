...
export default function (pi: ExtensionAPI) {
  ...
  const manager = new SubagentManager({
    createSubagentSession: (params) => params.backend === "cursor"
      ? createCursorSubagentSession(params, {
        exec: (cmd, args, opts) => pi.exec(cmd, args, opts),
        detectEnv,
        deriveSessionDir: deriveSubagentSessionDir,
        registry,
        assemblerIO: { buildAgentPrompt },
        lifecycle: subagentSessionDeps.lifecycle,
        onStderr: (text) => debugLog("cursor acp stderr", text.trimEnd()),
      })
  ...
  // Consumers: const { getSubagentsService } = await import("pi-bstn-subagents");
