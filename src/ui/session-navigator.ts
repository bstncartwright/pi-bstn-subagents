...
export class TranscriptOverlay implements Component {
  ...
  private rebuild(): Container {
    if (this.source.kind === "text") {
      return buildTextTranscriptComponents(this.source.getTextEntries(), this.theme);
    }
...
function buildTextTranscriptComponents(
  entries: readonly import("#src/lifecycle/child-session").TextTranscriptEntry[],
  theme: Theme,
): Container {
    ...
    const label = entry.role === "assistant"
      ...
      : entry.role === "user"
        ...
        : entry.role === "thought"
          ...
          : entry.role === "tool"
            ? theme.fg("toolTitle", entry.toolName ?? "Tool")
            : theme.fg("muted", "System");
    const status = entry.status ? theme.fg("dim", ` · ${entry.status}`) : "";
    container.addChild(new Text(`${label}${status}\n${entry.text || theme.fg("dim", "(no text)")}`, 0, 0));
