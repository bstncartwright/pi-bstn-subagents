...
export function renderFinishedLine(
	...
	const backendTag = ` ${theme.fg("dim", `(${agent.backend})`)}`;
	return `${icon} ${theme.fg("dim", name)}${backendTag}${modeTag}  ${theme.fg("dim", agent.description)} ${theme.fg("dim", "·")} ${theme.fg("dim", parts.join(" · "))}${statusText}`;
}
...
export function renderRunningLines(
	...
	const header = `${theme.fg("accent", frame)} ${theme.bold(name)}${backendTag}${modeTag}  ${theme.fg("muted", agent.description)} ${theme.fg("dim", "·")} ${theme.fg("dim", statsText)}`;
...
function buildSections(
	...
	const queuedBackends = (["pi", "cursor"] as const)
		.map(backend => ({ backend, count: categories.queued.filter(agent => agent.backend === backend).length }))
		.filter(({ count }) => count > 0)
		.map(({ backend, count }) => categories.queued.length === 1 ? backend : `${count} ${backend}`)
		.join(" · ");
	const queuedLine = categories.queued.length > 0
		? truncate(theme.fg("dim", "\u251C\u2500") + ` ${theme.fg("muted", "\u25E6")} ${theme.fg("dim", `${categories.queued.length} queued · ${queuedBackends}`)}`)
		: undefined;
